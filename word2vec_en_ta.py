...
model_en = gensim.models.KeyedVectors.load_word2vec_format(r"E:\models\wiki.en\wiki.en.vec")
model_ta = gensim.models.KeyedVectors.load_word2vec_format(r"E:\models\wiki.ta\wiki.ta.vec")
...
def most_similar_vect(self, vect_enter, top_n=10):
    ...
    dists = np.dot(np.squeeze(np.asarray(self.vectors)), np.squeeze(np.asarray(vect_unit)))
    if not top_n:
        return dists
...
def traducwithsco_ta_to_en(w, numb=10):
    return most_similar_vect(model_en, clf_ta_en.predict(model_ta[w].reshape(1, -1)), numb)
...
def traducwithsco_en_to_ta(w, numb=10):
    return most_similar_vect(model_ta, clf_en_ta.predict(model_en[w].reshape(1, -1)), numb)
